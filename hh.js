window.MAIN = {
  ...
  ChannelService: {
        ...
        fetch(t.url)
          ...
          .then(t => {
            let s = (new DOMParser).parseFromString(t, 'text/html'),
              ...
              i = n.props.initialState.note.data.comments_count;
            e.success({
              read: o,
              like: a,
              comment: i,
      ...
      articleData: (e, t) => {
        fetch(t.url)
          ...
          .then(t => {
            let s = (new DOMParser).parseFromString(t, 'text/html'),
              n = 0,
              o = s.querySelectorAll('div.article-box__meta div.item-list div.lm')[1].outerText.replace('阅读数', '')
                .trim();
            n = -1 != o.indexOf('.') ? o.replace('.', '')
              .replace('K', '00')
              .replace('W', '000') : o.replace('K', '000')
              .replace('W', '0000');
            const a = s.querySelectorAll('div.article-box__content div.action-box div.action-item__text');
            let i = a[1].outerText.replace('赞', '')
              .trim(),
              r = a[2].outerText.replace('收藏', '')
                .trim();
            e.success({
              read: n,
              like: i,
        ...
        fetch(`http://mp.toutiao.com/mp/agw/statistic/v2/item/info?item_id=${s}&type=1`)
          ...
          .then(t => {
            if (console.log(t), 'success' === t.message) {
              const { item_data: s = {} } = t || {}, { item_stat: n = {} } = s, { consume_data: o = {}, interaction_data: a = {} } = n, { go_detail_count: i = 0 } = o, { comment_count: r = 0, digg_count: c = 0 } = a;
              e.success({
                read: i,
                like: c,
                comment: r,
      ...
      articlePublish: (e, t) => {
        ...
        MAIN.ChannelService.zhihu.token(t => {
          fetch(`https://zhuanlan.zhihu.com/api/autocomplete/topics?token=${encodeURIComponent(n.topic)}&max_matches=5&use_similar=0&topic_filter=1`)
            ...
            .then(n => {
              Promise.resolve()
                .then(() => i.length > 0 ? Array.prototype.slice.call(i)
                  .reduce((s, n, o) => {
                    let i = n.getAttribute('src');
                    return i.startsWith('http') ? s.then(() => s.then(() => fetch(i)
                      .then(e => e.blob())
                      .then(e => {
                        let s = new FormData;
                      ...
                      .catch(() => e.error(`发布失败，图片不支持跨域 ${i}`)))) : s.then(() => Promise.resolve());
                  }, Promise.resolve()) : Promise.resolve())
                .then(() => {
                  let t = {
    ...
    weibo: {
      ...
      articlePublish: (e, t) => {
        let s = t.article, { isreward: n = 0, follow_to_read: o = 0 } = t.config || {},
          a = marked(s.content),
          i = (new DOMParser).parseFromString(a, 'text/html')
            .getElementsByTagName('img');
        fetch('https://weibo.com/')
          .then(t => {
            if (t.redirected) {
              const { url: r = '' } = t,
                c = r.split('?')[0].match(/\d+/g)[0];
              fetch(`https://card.weibo.com/article/v3/aj/editor/draft/create?uid=${c}`, { method: 'POST' })
                .then(e => e.json())
                .then(t => {
                  if (1e5 === t.code) {
                    const r = t.data || {};
                    fetch(`https://card.weibo.com/article/v3/aj/editor/draft/save?uid=${c}&id=${r.id}`, {
                      method: 'POST',
                      headers: X_FORM_HEADER,
                      body: [`id=${r.id}`, `title=${s.title}`, `updated=${r.updated}`, `subtitle=`, `type=`, `status=0`, `publish_at=`, `error_msg=`, `error_code=0`, `collection=[]`, `free_content=`, `content=${encodeURIComponent(a)}`, `cover=${encodeURIComponent(i.length && i[0].src ? i[0].src : 'https://wx4.sinaimg.cn/large/abd13f1bly1fjb28knztnj20ku0bqwh5.jpg')}`, `writer=`, `extra=[]`, `is_word=0`, `article_recommend=[]`, `follow_to_read=${o}`, `follow_to_read_detail[result]=1`, `follow_to_read_detail[x]=0`, `follow_to_read_detail[y]=0`, `follow_to_read_detail[readme_link]=${encodeURIComponent(`http://t.cn/A6UnJsqW`)}`, `follow_to_read_detail[level]=`, `isreward=${n}`, `pay_setting={"ispay":0,"isvclub":0}`, `source=0`, `action=0`, `content_type=0`, `save=1`].join('&'),
                    })
                      .then(e => e.json())
                      .then(t => {
                        console.log('文章保存成功！', t), fetch(`https://card.weibo.com/article/v3/aj/editor/draft/publish?uid=${c}&id=${r.id}`, {
                          method: 'POST',
                          headers: X_FORM_HEADER,
                          body: [`id=${r.id}`, `rank=0`, `text=${s.title}`, `follow_official=1`, `sync_wb=0`, `is_original=0`, `time=`].join('&'),
                        })
                          .then(e => e.json())
                          .then(t => {
    ...
    baijiahao: {
      verifyCookie: e => {
        fetch('https://baijiahao.baidu.com/builder/app/appinfo')
          .then(e => e.json())
          .then(t => {
            e.success('success' === t.errmsg);
          })
          .catch(t => {
            console.warn('百家号认证失败：', t), e.success(!1);
          })
      },
      verifyChannelConfig: (e, t) => {
      },
